...
def reset_password(email):
    ...
    send_btn = col_2.form_submit_button("Send", use_container_width=True,type= 'primary',icon=AppIcons.SEND_EMAIL)
...
if 'user_info' not in st.session_state:
    ...
    if register and auth_form.form_submit_button(label='Create Account',use_container_width=True,type='primary',icon=AppIcons.SEND_EMAIL):
        with auth_notification, st.spinner('Creating account'):
            auth.create_account(email,password)
     # Sign In
    elif register == False and auth_form.form_submit_button(label='Sign In',use_container_width=True,type='primary',icon=AppIcons.LOG_IN):

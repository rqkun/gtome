...
def reset_password(email):
    ...
    reset_email = col_1.text_input(label='Email',value=email)
    send_btn = col_2.form_submit_button("Send", use_container_width=True,type= 'primary',icon=":material/send:")
...
if 'user_info' not in st.session_state:
    ...
    left.markdown("""
                        <div style="text-align: center; font-size: 50px; font-weight: bold;">GTOME</div>
    ...
    if bot_right.button("Reset",type= "secondary",use_container_width=True,icon=":material/mail:"):
        reset_password(email)
    
    
    if register and auth_form.form_submit_button(label='Create Account',use_container_width=True,type='primary',icon=":material/edit_square:"):
        with auth_notification, st.spinner('Creating account'):
            auth.create_account(email,password)
     # Sign In
    elif register == False and auth_form.form_submit_button(label='Sign In',use_container_width=True,type='primary',icon=":material/input:"):
